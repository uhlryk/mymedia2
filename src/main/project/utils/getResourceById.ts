...
export const getResourceById = async (specificProject: SpecificProject, projectPath: string, resourceId: string): Promise<IResource> => {
    if (
        !specificProject ||
        !specificProject.verifyProjectPath(projectPath)
    ) {
        console.error(
